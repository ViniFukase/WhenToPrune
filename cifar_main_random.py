...
parser.add_argument('--cp', action='store_true', help='train with critical period detection')
...
# New function to get convolutional layer names (adapted from TensorFlow code)
def get_conv_layer_names(model):
    layer_names = []
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) and module.weight.requires_grad:
            layer_names.append(name)
    safe_print("Convolutional layer names:", layer_names)
    return layer_names

# New function to calculate cosine distance (adapted from TensorFlow code)
def calculate_cosine_distance(initial_weights, current_weights):
    distances = []
    for w1, w2 in zip(initial_weights, current_weights):
        flat_w1 = w1.flatten().cpu().numpy()
        flat_w2 = w2.flatten().cpu().numpy()
        distance = cosine(flat_w1, flat_w2)
        distances.append(distance)
    return np.mean(distances)

# New function to check rotation angle criterion (adapted from TensorFlow code)
from sklearn.preprocessing import MinMaxScaler

def check_rotation_angle_criterion(initial_weights, current_weights, cosine_distances, epoch, window_size=5, total_epochs=200):
    global cosine_distance
    global angle
    cosine_distance = calculate_cosine_distance(initial_weights, current_weights)
    cosine_distances.append(cosine_distance)
    angle = float('nan')
    safe_print(f"Mean cosine distance: {cosine_distance:.4f}")

    if len(cosine_distances) >= window_size:
        recent_distances = cosine_distances[-window_size:]
        
        # Normalize epochs
        epochs_ndarray = np.arange(1, total_epochs + 1).reshape(-1, 1)
        scaler_epochs = MinMaxScaler(feature_range=(0, 1))
        epochs_norm = scaler_epochs.fit_transform(epochs_ndarray)
        epochs_norm_1d = epochs_norm.flatten()
        if epoch >= window_size:
            epochs_window = epochs_norm_1d[epoch - window_size:epoch]  # 0-based indexing
        else:
            angle = float('nan')
            return False
        
        # safe_print("Epochs_window:", epochs_window)
        # safe_print("Recent distances:", recent_distances)
        
        slope, intercept = np.polyfit(epochs_window, recent_distances, 1)
        angle = np.degrees(np.arctan(slope))
        safe_print(f"Angle: {angle:.2f}°")
        if angle < 45:
            return True
...
def main(args):
    ...
    all_best_accuracies = []
    all_saved_ratios = []
    weights_dir = f'./weights/{args.dataset}/{args.model}'
    ...
    for iternum in range(args.num_iterations):
        ...
        with open(csvfile, 'w', newline='') as file:
            ...
            writer.writerow(['Epoch', 'Len of Training set', 'Avg Training Loss', 'Avg Test Loss', 'TestAcc', 'Learning Rate', 'Mean Cosine Distance', 'Angle', 'Threshold'])
        ...
        cosine_distances = []
        # cp_metrics = []  # Lista de tuplas (epoch, lr, cosine_dist, angle)
        # cp_end_epoch = None
        if args.cp:
            conv_layer_names = get_conv_layer_names(model)
            initial_weights = [model.get_parameter(name + '.weight').detach().clone() for name in conv_layer_names]
            # trainset_full = full_train_dataset  # Keep full dataset initially
            # trainloader = full_train_loader
        ...
        for epoch in range(args.epochs):
            ...
            # Aplicar verificação do CP, se não passar threshold = 0, se passar = valor setado no args.
            # Critical period logic
            # if args.cp and cp_end_epoch is None:
            if args.cp:
                current_weights = [model.get_parameter(name + '.weight').detach().clone() for name in conv_layer_names]
                if check_rotation_angle_criterion(initial_weights, current_weights, cosine_distances, epoch + 1, window_size=5, total_epochs=args.epochs):
                    threshold1 = threshold
                else:
                    threshold1 = 0
            else:
            ...
            if threshold1 > 0:
                # Lógica de Data Pruning Aleatório de 50%
                # -----------------------------------------------------------
                
                # 1. Obtém a lista atual de índices
                current_indices = train_loader.dataset.index_list
                total_current_samples = len(current_indices)
                
                # 2. Calcula o número de amostras a serem mantidas (50%)
                PRUNING_FRACTION_TO_KEEP = 0.5 
                num_to_keep = int(total_current_samples * PRUNING_FRACTION_TO_KEEP)
                
                # Garante que não zere a lista (mantém pelo menos 1 amostra se a lista não estiver vazia)
                if num_to_keep == 0 and total_current_samples > 0:
                    num_to_keep = 1
                elif num_to_keep == 0:
                    safe_print("Lista de treinamento vazia. Pulando pruning.")
                    saved_ratio = 1.0 
                    continue

                # 3. Seleciona aleatoriamente 50% dos índices
                new_indices = random.sample(current_indices, num_to_keep)
                
                # 4. Cria um novo dataset com os índices selecionados
                # Utilizamos full_train_dataset.data/labels pois eles são as fontes de dados originais.
                pruned_train_dataset = cifar_dataset( 
                ...
                # 5. Recria o DataLoader
                train_loader = DataLoader(
                    dataset=pruned_train_dataset, 
                ...
                # 6. Atualiza o contador de amostras salvas
                total_removed_samples = total_current_samples - num_to_keep
                total_saved_samples += total_removed_samples
                
                saved_ratio = total_removed_samples / total_current_samples if total_current_samples > 0 else 0
                
                safe_print(f"Data Pruning Aleatório Aplicado: {total_current_samples} -> {num_to_keep} amostras mantidas. Ratio de redução nesta época: {(1 - PRUNING_FRACTION_TO_KEEP) * 100:.2f}%")
                # -----------------------------------------------------------
            else:
                saved_ratio = 0
            ...
            with open(csvfile, 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(
                    [epoch + 1, len(train_loader.dataset), epoch_loss, avg_test_loss, correct, current_lr, cosine_distance, angle, threshold1])
        ...
        print(f"\nSaving model weights. Epoch 200.\n")
        if args.cp:
            weights_path = os.path.join(weights_dir, f'{args.dataset}_{args.model}_epoch_200_CP_{iternum}.pth')
        elif args.threshold == 0:
            weights_path = os.path.join(weights_dir, f'{args.dataset}_{args.model}_epoch_200_baseline_{iternum}.pth')
        else:
            weights_path = os.path.join(weights_dir, f'{args.dataset}_{args.model}_epoch_200_IES_{iternum}.pth')
