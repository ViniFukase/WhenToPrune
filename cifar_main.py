...
def main(args):
    ...
    for iternum in range(args.num_iterations):
        ...
        for epoch in range(args.epochs):
            ...
            #     # Cálculo do ângulo com base na função já existente
            #     angle = None
            #     if len(cosine_distances) >= 5:
            #         angle = return_angle(initial_weights, current_weights, cosine_distances, epoch + 1, window_size=5, total_epochs=args.epochs)
            #     else:
            #         angle = float('nan')

            #     # Pegando learning rate atual
            #     current_lr = optimizer.param_groups[0]['lr']

            #     # cp_metrics.append((epoch + 1, current_lr, cosine_distance, angle))  # epoch é zero-based
